...
We implement the Reflected MYULA algorithm described in: "Efficient Bayesian Computation 
...
path = '../bin_denoising_MIV_0.1_t100_RMYULA'
...
t = (torch.zeros([nx,ny], dtype=int) + 100).to(device).detach().clone()
...
theta = 60               # hyperparameter of the prior
...
RMYULA_trace=torch.zeros([store_samples,nx,ny])
k=0
...
RMYULA_results = {"theta": theta,
	"NRMSE_trace_RMYULA": NRMSE_trace,
	"NRMSE_RMYULA": NRMSE(post_meanvar.get_mean().cpu().numpy()),
	"RSNR_RMYULA": RSNR(post_meanvar.get_mean().cpu().numpy()),
    "PSNR_RMYULA": PSNR(post_meanvar.get_mean().cpu().numpy()),
    "SSIM_RMYULA": SSIM(post_meanvar.get_mean().cpu().numpy()),
	"logPiTrace_RMYULA" : logPiTrace,
	"neg_values_RMYULA": min_values_trace,
	"meanSamples_RMYULA": post_meanvar.get_mean().cpu().numpy(),
	"variance_RMYULA": post_meanvar.get_var().cpu().numpy()}

RMYULA_MC_chain = {"RMYULA_MC_chain": RMYULA_trace.cpu().numpy()}

savemat(path + "/bin_denoising_MIV_0.1_t100_RMYULA_results.mat", RMYULA_results)
savemat(path + "/bin_denoising_MIV_0.1_t100_RMYULA_MC_chain.mat", RMYULA_MC_chain)
