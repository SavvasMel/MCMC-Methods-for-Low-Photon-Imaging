...
path = '../bin_denoising_MIV_0.1_t100_RSKROCK'
...
L_F = torch.max(y)*(torch.max(h)**2)*torch.exp(-torch.min(b))/((1-torch.exp(-torch.min(b)))**2)   # Lipschitz constant
...
RSKROCK_results = {
	"theta": theta,
	"NRMSE_trace_SKROCK": NRMSE_trace,
	"NRMSE_SKROCK": NRMSE(post_meanvar.get_mean().cpu().numpy()),
	"RSNR_SKROCK": RSNR(post_meanvar.get_mean().cpu().numpy()),
    "PSNR_SKROCK": PSNR(post_meanvar.get_mean().cpu().numpy()),
    "SSIM_SKROCK": SSIM(post_meanvar.get_mean().cpu().numpy()),
	"logPiTrace_SKROCK" : logPiTrace,
	"neg_values_SKROCK": min_values_trace,
	"meanSamples_SKROCK": post_meanvar.get_mean().cpu().numpy(),
	"variance_SKROCK": post_meanvar.get_var().cpu().numpy()}

RSKROCK_MC_chain={"RSKROCK_MC_chain": RSKROCK_trace.cpu().numpy()}

savemat(path + "/bin_denoising_MIV_0.1_t100_RSKROCK_results.mat", RSKROCK_results)
savemat(path + "/bin_denoising_MIV_0.1_t100_RSKROCK_MC_chain.mat", RSKROCK_MC_chain)
