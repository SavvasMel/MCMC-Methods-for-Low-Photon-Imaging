...
def RSKROCK(X: torch.Tensor,Lipschitz,nStages: int,eta_sk,dt_perc,grad_Phi):
    ...
    dtSKROCK = dt_perc*rhoSKROCK/Lipschitz # step-size
    ...
    Xts= torch.abs(X.clone() - mu1*dtSKROCK*grad_Phi(torch.abs(X + nu1*Q)) + kappa1*Q)
    ...
    for js in range(2,nStages+1): # s=2,...,nStages SK-ROCK internal iterations
        ...
        Xts= -mu*dtSKROCK*grad_Phi(Xts) + nu*Xts + kappa*XtsMinus2
