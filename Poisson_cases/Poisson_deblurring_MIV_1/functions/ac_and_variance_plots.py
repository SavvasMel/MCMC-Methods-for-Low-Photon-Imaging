...
def ac_var_plots(MC_chain, lags, method_str, path, img_type):
    ...
    MC_chain_fourier = np.abs(np.fft.fft2(MC_chain))
    variance_array_fourier = np.var(MC_chain_fourier, axis=0)

    ind_min_variance = np.argmin(variance_array_fourier.ravel()) 
    chain_elem_min_variance = MC_chain_fourier.reshape(MC_chain_fourier.shape[0],-1)[:,ind_min_variance]

    ind_max_variance = np.argmax(variance_array_fourier.ravel()) 
    chain_elem_max_variance = MC_chain_fourier.reshape(MC_chain_fourier.shape[0],-1)[:,ind_max_variance]
    
    ind_median_variance = np.argsort(variance_array_fourier.ravel())[len(variance_array_fourier.ravel())//2]
    chain_elem_median_variance = MC_chain_fourier.reshape(MC_chain_fourier.shape[0],-1)[:,ind_median_variance]

    matplotlib.rc('figure', figsize=(15, 15))
    matplotlib.rc('font', size=30)
    fig,ax = plt.subplots()
    plot_acf(chain_elem_median_variance,ax=ax,label='Median-speed component',alpha=None,lags=lags)
    plot_acf(chain_elem_max_variance,ax=ax,label='Slowest component',alpha=None,lags=lags)
    plot_acf(chain_elem_min_variance,ax=ax,label='Fastest component',alpha=None,lags=lags)
