...
We implement the Reflected SKROCK algorithm described in: "Efficient Bayesian Computation 
...
path = '../geo_inpainting_MIV_0.1_RSKROCK'
...
theta = torch.tensor(43)        # hyperparameter of the prior
...
grad_Phi = lambda x: gradF(x) + gradG(x)
...
store_samples= int(50)    # thinning samples
...
for i_x in tqdm(range(1,int(nSamples_burnin+nSamples))):
    ...
    X, min_value = RSKROCK(X, L_Phi, stages_skrock, eta_skrock, percDeltat, grad_Phi, device)
...
savemat(path + "/geo_inpainting_MIV_0.01_RSKROCK_results.mat", RSKROCK_results)
savemat(path + "/geo_inpainting_MIV_0.01_RSKROCK_MC_chain.mat", RSKROCK_MC_chain)
