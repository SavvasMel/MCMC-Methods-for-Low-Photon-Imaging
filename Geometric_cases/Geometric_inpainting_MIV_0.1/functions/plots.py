...
def plots(z_true, MaT_f, t, post_meanvar, NRMSE_trace, logPiTrace, min_val_trace, path, img_type, alpha_trace = None):
    ...
    plt.imshow(np.log((MaT_f(t)).cpu().numpy()+1),cmap="gray")
    plt.savefig(path + "/noisy.png", bbox_inches="tight", dpi=300)
    plt.close()
